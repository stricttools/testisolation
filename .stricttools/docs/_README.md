...
|-----------|---------|----------|
| **Pytest plugin** | `pip install testisolation` | [python/](python/) |
| **Go env-hygiene module** | `go get github.com/stricttools/testisolation/go` | [go/](go/) |
| **Node env-hygiene package** | `npm install --save-dev testisolation` | [typescript/](typescript/) |
...
testisolation_sockets = "deny"              # "deny" | "allowlist"
testisolation_socket_allowlist = []         # ["example.com:443", "[::1]:5432"]
testisolation_unix_socket_allowlist = []    # ["/run/pg/.s.PGSQL.5432"]
testisolation_loopback = "deny"             # "deny" | "allow"
testisolation_sandbox_required = "false"    # "true" once a sandbox runner exists
```
...
| Key | Default | Meaning |
|-----|---------|---------|
| `testisolation_threshold` | `50` | Bare-run refusal threshold |
| `testisolation_sandbox_env` | `TESTISOLATION_SANDBOX` | Env var the sandbox runner sets to `1` |
| `testisolation_runner_command` | `scripts/test.sh` | Command named in the refusal message |
| `testisolation_tmp_prefix` | `testisolation-env-` | Prefix of the throwaway env directory |
| `testisolation_git_user_name` | `testisolation` | Throwaway commit identity |
| `testisolation_git_user_email` | `testisolation@example.invalid` | Throwaway commit identity |
| `testisolation_preserve` | *(empty)* | Toolchain caches preserved across the HOME repoint |

`testisolation_preserve` accepts only a closed enum of known-safe toolchain
...
`testisolation.pgcluster` boots a throwaway cluster for suites that need a real
...
`testisolation_unix_socket_allowlist` (e.g. `/dev/shm/`). `psycopg` and everything
